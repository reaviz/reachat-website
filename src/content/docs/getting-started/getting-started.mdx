...
defining design tokens in your CSS with Tailwind v4's `@theme` directive.

For example, if we wanted to change the default blue to a purple, we could
override the primary color tokens like this:

```css copy
@theme {
  --color-primary: #a855f7;        /* purple-500 */
  --color-primary-hover: #9333ea;  /* purple-600 */
  --color-secondary: #d8b4fe;      /* purple-300 */
}
```
...
- Markdown display with custom remark plugins
- Multi-file uploads with type-aware renderers (images, CSV, PDF, etc.)
- Source citations
- Auto-scroll and scroll-to-bottom interactions
- `@` mentions and `/` slash commands
- LLM-driven UI via the [Component Catalog](/docs/examples/component-catalog)
...
Most components in the chat tree are slot-based — you can swap them out for your
own React components without giving up the rest of reachat's behavior. See the
[Custom Components](/docs/customization/custom) guide and the matching
