...
The Component Catalog lets LLMs render **custom React components** in chat
...
[reaviz](https://github.com/reaviz/reaviz) that you can register in your
catalog with `createChartComponentDef()`. It supports bar, line, area, pie,
and sparkline chart types out of the box.

See the [Charts](/docs/examples/charts) page for setup, configuration, and
examples.
...
Two ways to wire a catalog in, depending on how much control you need:

```mermaid
flowchart TD
    A[Need custom plugins or overrides?] -->|No| B[Pass components=catalog]
    A -->|Yes| C[Pass remarkPlugins + markdownComponents]
```
...
```mermaid
flowchart LR
    A[JSON parse] --> B[Structure check]
    B --> C[Catalog lookup]
    C --> D[Zod validation]
    D --> E[Recursive children]
    E -.->|repeat 2-4 per child| B
```
